# TODO

- [x] 增加功能：Replace冲突处理策略的目标文件重命名可由用户决定规则。
- [x] 增加功能：用户可选，当有失败项时操作完成后是否关闭对话框。
- [x] 完成功能：ConflictDecisionDialog，“跳过相同日期与大小”可选框逻辑。
- [ ] ~~完成功能：ConflictDecisionDialog，表头可选框与表头打开外部链接逻辑。~~
- [x] 完成功能：ConflictDecisionDialog，增加CheckBox模拟表头。
- [ ] ~~完成功能：FileLinkWorker::isOnSameDriver，其他平台的逻辑实现。~~（重实现Replace逻辑以摒弃此操作）
- [x] 优化功能：增加Windows平台下长路径支持。
- [ ] ~~优化功能：FileLinkWorker的文件收集与文件处理异步执行，降低内存使用量（不需要存储所有收集到的条目列表然后再处理）。~~（使用生产者消费者模型可解决，不过感觉没有必要引入新线程，目前的内存使用量还算可以接受；此外，如果采用异步线程，其也要先计算总文件数量以供进度条能够正常显示，有点鸡肋）
- [ ] ~~增加实用程序，对给定的目录，将其中所有符合用户筛选器的文件进行硬链接替换。~~（没有必要）
- [x] 优化大量失败条目产生时的程序性能。
- [x] 修复Bug，当URL是磁盘根目录时，无法打开外部链接，且文本样式不正确。
- [ ] 改善Windows下Shortcut类型条目的链接行为。
- [ ] ~~优化MacOS下的进度条显示。~~
- [x] 解决MacOS下获取焦点访达窗口所在目录路径失败的问题。
- [x] 解决MacOS下程序初次运行需要获取“辅助功能”权限时，GHM启动失败导致热键注册失败的问题。（加入权限检测与授予权限弹窗）
- [x] 修复KeyCombination Inputer在MacOS上Ctrl与Meta键不一致的问题。
- [ ] ~~修复MacOS上不在控制台启动应用时热键失效的问题。~~（权限获取失败）
- [x] 加入文件日志记录。
- [x] 使用Service Management替代plist实现开机自启动。
- [x] 解决MacOS下脚本文件配置失败的问题。
- [ ] ~~解决MacOS上有时权限已授予但检测为未授予权限的问题。~~
- [x] 修复MacOS下某些时候程序启动多个实例的问题（初步判断与“开机自启动”脚本相关）（main函数中的LockFile被销毁了）
- [ ] ~~修复MacOS下“开机自启动”脚本成功运行，但日志文件中输出为失败的问题。~~
- [ ] ~~修复MacOS下权限获取的相关问题（获取权限时需要先移除“非预期存在且启用”的EasyLinks项，而后再重新通过程序的权限获取窗口启用新的EasyLinks项，权限才会成功获取）。~~（构建的程序使用了临时签名证书导致的）
- [x] 完成MacOS下的程序开机自启动功能。（设置开机自启动失败原因：ad-hoc证书签名问题）
- [x] 更新程序运行截图。
- [x] 优化程序图标。
- [x] 增加子功能：选定多个文件夹，将其中所有符合筛选规则（例如相同文件名，相同文件大小）的子文件（递归式）进行链接。
